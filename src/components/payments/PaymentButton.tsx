...
export function PaymentButton({ sessionId, amount, skillTitle, className = "" }: PaymentButtonProps) {
  ...
  const verifyMutation = useMutation({
    ...
    onError: () => {
      ...
      setRazorpayLoading(false); // ensure spinner clears if modal was dismissed first
    },
    onSettled: () => {
  ...
  // Pre-load Razorpay script when breakdown modal opens — reduces checkout latency
  function handlePayNowClick() {
    loadRazorpayScript(); // fire-and-forget pre-load; result awaited again in onSuccess
