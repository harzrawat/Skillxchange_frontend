...
import { Calendar, Coins, IndianRupee, CheckCircle, AlertCircle, Video } from "lucide-react";
...
function SessionCard({
  ...
  const isScheduledOrInSession = match.session?.status === "scheduled" || match.session?.status === "in_session";
    ...
    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 flex flex-col gap-4">
      ...
      <div className="flex items-start justify-between gap-3">
        ...
        <div className="flex items-center gap-2 shrink-0">
          ...
          >
            {isCompleted ? "Completed" : (match.session?.status === "in_session" ? "In Session" : "Scheduled")}
          </span>
        </div>
      </div>
      ...
      {isScheduledOrInSession && match.sessionType === "credit" && isLearner && (
      ...
      {/* Actions */}
      {isScheduledOrInSession && match.session?.sessionId && (
        <div className="flex gap-2 pt-1">
          {/* Join Class button */}
          <button
            onClick={() => navigate(`/sessions/${match.session!.sessionId}/classroom`)}
            disabled={match.sessionType === "paid" && !match.session.isPaid}
            className={`flex-1 flex items-center justify-center gap-2 rounded-xl px-4 py-2 text-sm font-semibold transition ${
              match.sessionType === "paid" && !match.session.isPaid 
                ? "bg-gray-200 text-gray-500 cursor-not-allowed"
                : "bg-indigo-600 text-white hover:bg-indigo-500"
            }`}
            title={match.sessionType === "paid" && !match.session.isPaid ? "Complete payment to join" : ""}
          >
          ...
          {match.sessionType === "paid" && isLearner && !match.session?.isPaid && (
            <PaymentButton
...
export default function MySessionsPage() {
  ...
  const scheduledSessions = (acceptedMatches ?? []).filter(
    (m) => m.session && (m.session.status === "scheduled" || m.session.status === "in_session")
  );
